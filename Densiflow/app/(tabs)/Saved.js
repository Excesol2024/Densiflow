...
const Saved = () => {
  ...
  const fetchSavedPlaces = async () => {
    try {
      ...
      setFilteredPlaces(places); // Initialize filteredPlaces with all places
    } catch (error) {
      console.log(error);
    }
  };
      ...
      <View className="flex-1 p-3 mt-8 mb-20">
        <View className="rounded-full shadow-lg shadow-gray-900 bg-white mb-3">
          <View className="flex-row items-center bg-transparent rounded-full p-2">
            <AntDesign name="search1" size={27} color="gray" paddingLeft={5} />
            <TextInput
              style={{ fontFamily: "PoppinsThin" }}
...
${
  places.crowd_status === "high"
    ...
    : ""
                ...
                ></View>
                <Image source={{ uri: places.image_url }} className="rounded-2xl w-20 h-20" />
                <View className="flex-1 pl-2">
                  <Text style={{ fontFamily: "PoppinsBold" }} className="text-md pl-1">
                    {places.name}
                  </Text>
                  <Text style={{ fontFamily: "PoppinsBold" }} className="text-md pl-1">
                    {places.subaddress}
